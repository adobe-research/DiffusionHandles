...
class Attention(nn.Module):
    ...
    def get_attention_scores_(self, query, key, attention_mask=None):
...
class AttnProcessor:
    ...
    def __call__(
        ...
        #attention_probs_ = attn.get_attention_scores_(query, key, attention_mask)        
...
class AttnProcessor2_0:
    ...
    def sdp_attention(self, query, key, value, attn_mask=None, dropout_p=0.0, is_causal=False, scale=None) -> torch.Tensor:
    ...
    def __call__(
        ...
        hidden_states = F.scaled_dot_product_attention(
            query, key, value, attention_mask, dropout_p=0.0, is_causal=False
        )

        #dummy_value = torch.stack([torch.stack([torch.eye(key.shape[-2])] * hidden_states.shape[1])] * hidden_states.shape[0]).to(value.device, value.dtype)
        
        dummy_value = torch.eye(key.shape[-2]).to(value.device, value.dtype)

        #print(dummy_value.shape)

        attn_probs = F.scaled_dot_product_attention(
            query, key, dummy_value[None, None, :, :], attention_mask, dropout_p=0.0, is_causal=False
        ) 
        
        #attn_probs = attn_probs.flatten(start_dim = 0, end_dim = 1)
        ...
        return hidden_states, attn_probs[0]
